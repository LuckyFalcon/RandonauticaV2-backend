...
app.get("/api/signInUser", [
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      //Insert user
      await UserInserts.InsertUser(req.user)
      .then(async() => {
        //Insert user details
        await UserInserts.InsertUserDetails(req.user, platform)
          .then(async() => {
            //Select curent user
            await UserInserts.SelectUserDetails(req.user, platform).then(async(result) => {
